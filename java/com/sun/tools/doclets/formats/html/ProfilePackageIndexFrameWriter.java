...
public class ProfilePackageIndexFrameWriter extends AbstractProfileIndexWriter {
   ...
   */
  public ProfilePackageIndexFrameWriter(ConfigurationImpl configuration, DocPath filename)
      throws IOException {
  ...
  protected Content getPackage(PackageDoc pd, String profileName) {
    ...
    if (pd.name().length() > 0) {
      ...
      packageLinkContent =
          getHyperLink(
              pathString(pd, DocPaths.profilePackageFrame(profileName)),
              pkgLabel,
              "",
              "packageFrame");
    } else {
      pkgLabel = new StringContent("<unnamed package>");
      packageLinkContent = getHyperLink(DocPaths.PACKAGE_FRAME, pkgLabel, "", "packageFrame");
    }
