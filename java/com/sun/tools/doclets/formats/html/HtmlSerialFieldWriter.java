...
public class HtmlSerialFieldWriter extends FieldWriterImpl
  ...
  public void addMemberHeader(
    ...
    if (fieldType == null) {
      ...
      Content fieldContent =
          writer.getLink(
              new LinkInfoImpl(configuration, LinkInfoImpl.Kind.SERIAL_MEMBER, fieldType));
  ...
  public boolean shouldPrintOverview(FieldDoc field) {
    if (!configuration.nocomment) {
      if (!field.commentText().isEmpty() || writer.hasSerializationOverviewTags(field)) return true;
    }
    if (field.tags("deprecated").length > 0) return true;
    return false;
  }
}
