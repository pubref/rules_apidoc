...
public class ConstantsSummaryWriterImpl extends HtmlDocletWriter implements ConstantsSummaryWriter {
  ...
  public ConstantsSummaryWriterImpl(ConfigurationImpl configuration) throws IOException {
    ...
    constantsTableSummary =
        configuration.getText(
            "doclet.Constants_Table_Summary", configuration.getText("doclet.Constants_Summary"));
   ...
   * {@inheritDoc}
   */
  public void addLinkToPackageContent(
      PackageDoc pkg,
    ...
    if (packageName.length() == 0) {
      link =
          getHyperLink(getDocLink(SectionName.UNNAMED_PACKAGE_ANCHOR), defaultPackageLabel, "", "");
    } else {
      Content packageNameContent = getPackageLabel(parsedPackageName);
      packageNameContent.addContent(".*");
      link = getHyperLink(DocLink.fragment(parsedPackageName), packageNameContent, "", "");
  ...
  public Content getContentsList(Content contentListTree) {
    Content titleContent = getResource("doclet.Constants_Summary");
    Content pHeading =
        HtmlTree.HEADING(HtmlConstants.TITLE_HEADING, true, HtmlStyle.title, titleContent);
   ...
   * {@inheritDoc}
   */
  public void addPackageName(PackageDoc pkg, String parsedPackageName, Content summariesTree) {
    Content pkgNameContent;
    if (parsedPackageName.length() == 0) {
      summariesTree.addContent(getMarkerAnchor(SectionName.UNNAMED_PACKAGE_ANCHOR));
    ...
    Content heading = HtmlTree.HEADING(HtmlConstants.PACKAGE_HEADING, true, pkgNameContent);
  ...
  public Content getConstantMembersHeader(ClassDoc cd) {
    ...
    Content classlink =
        (cd.isPublic() || cd.isProtected())
            ? getLink(new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CONSTANT_SUMMARY, cd))
  ...
  protected Content getClassName(Content classStr) {
    Content table =
        HtmlTree.TABLE(
            HtmlStyle.constantsSummary, 0, 3, 0, constantsTableSummary, getTableCaption(classStr));
  ...
  private Content getTypeColumn(FieldDoc member) {
    Content anchor = getMarkerAnchor(currentClassDoc.qualifiedName() + "." + member.name());
    ...
    Content type =
        getLink(new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CONSTANT_SUMMARY, member.type()));
  ...
  private Content getNameColumn(FieldDoc member) {
    Content nameContent =
        getDocLink(LinkInfoImpl.Kind.CONSTANT_SUMMARY, member, member.name(), false);
