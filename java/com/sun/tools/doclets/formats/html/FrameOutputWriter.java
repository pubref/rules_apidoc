...
public class FrameOutputWriter extends HtmlDocletWriter {
  ...
  protected void generateFrameFile() throws IOException {
    ...
    if (configuration.windowtitle.length() > 0) {
      printFramesetDocument(configuration.windowtitle, configuration.notimestamp, frameset);
    } else {
  ...
  protected void addFrameWarning(Content contentTree) {
    ...
    Content noframesHead =
        HtmlTree.HEADING(HtmlConstants.CONTENT_HEADING, getResource("doclet.Frame_Alert"));
    ...
    Content p =
        HtmlTree.P(
            getResource(
                ...
                getHyperLink(
                    configuration.topFile, configuration.getText("doclet.Non_Frame_Version"))));
  ...
  protected Content getFrameDetails() {
    HtmlTree frameset =
        HtmlTree.FRAMESET("20%,80%", null, "Documentation frame", "top.loadFrames()");
    if (noOfPackages <= 1) {
  ...
  private void addAllPackagesFrameTag(Content contentTree) {
    HtmlTree frame =
        HtmlTree.FRAME(
            DocPaths.OVERVIEW_FRAME.getPath(),
            "packageListFrame",
  ...
  private void addAllClassesFrameTag(Content contentTree) {
    HtmlTree frame =
        HtmlTree.FRAME(
            DocPaths.ALLCLASSES_FRAME.getPath(),
            "packageFrame",
