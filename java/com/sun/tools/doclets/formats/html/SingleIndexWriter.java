...
public class SingleIndexWriter extends AbstractIndexWriter {
  ...
  public static void generate(ConfigurationImpl configuration, IndexBuilder indexbuilder) {
    ...
    try {
      indexgen = new SingleIndexWriter(configuration, filename, indexbuilder);
  ...
  protected void addLinksForIndexes(Content contentTree) {
    for (int i = 0; i < indexbuilder.elements().length; i++) {
      ...
      contentTree.addContent(getHyperLink(getNameForIndex(unicode), new StringContent(unicode)));
