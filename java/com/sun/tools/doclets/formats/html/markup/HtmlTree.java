...
public class HtmlTree extends Content {
  ...
  public HtmlTree(HtmlTag tag, Content... contents) {
    this(tag);
    for (Content content : contents) addContent(content);
  }
  ...
  public void addAttr(HtmlAttr attrName, String attrValue) {
    if (attrs.isEmpty()) attrs = new LinkedHashMap<HtmlAttr, String>(3);
  ...
  public void addContent(Content tagContent) {
    if (tagContent instanceof ContentBuilder) {
      ...
      if (content.isEmpty()) content = new ArrayList<Content>();
  ...
  public void addContent(String stringContent) {
    if (!content.isEmpty()) {
      ...
      if (lastContent instanceof StringContent) lastContent.addContent(stringContent);
  ...
  public int charCount() {
    int n = 0;
    for (Content c : content) n += c.charCount();
    return n;
  }
  ...
  public static HtmlTree DIV(HtmlStyle styleClass, Content body) {
    ...
    if (styleClass != null) htmltree.addStyle(styleClass);
    return htmltree;
  }
  ...
  public static HtmlTree FRAME(String src, String name, String title, String scrolling) {
    ...
    if (scrolling != null) htmltree.addAttr(HtmlAttr.SCROLLING, scrolling);
    return htmltree;
  }
  ...
  public static HtmlTree FRAMESET(String cols, String rows, String title, String onload) {
    ...
    if (cols != null) htmltree.addAttr(HtmlAttr.COLS, cols);
    if (rows != null) htmltree.addAttr(HtmlAttr.ROWS, rows);
    htmltree.addAttr(HtmlAttr.TITLE, nullCheck(title));
   ...
   */
  public static HtmlTree HEADING(
      HtmlTag headingTag, boolean printTitle, HtmlStyle styleClass, Content body) {
    HtmlTree htmltree = new HtmlTree(headingTag, nullCheck(body));
    if (printTitle) htmltree.setTitle(body);
    if (styleClass != null) htmltree.addStyle(styleClass);
    return htmltree;
  }
  ...
  public boolean write(Writer out, boolean atNewline) throws IOException {
    if (!isInline() && !atNewline) out.write(DocletConstants.NL);
    ...
    out.write(">");
    boolean nl = false;
    for (Content c : content) nl = c.write(out, nl);
