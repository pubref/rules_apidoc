...
public abstract class AbstractTreeWriter extends HtmlDocletWriter {
   ...
   */
  protected AbstractTreeWriter(
      ConfigurationImpl configuration, DocPath filename, ClassTree classtree) throws IOException {
  ...
  protected void addLevelInfo(
      ClassDoc parent, List<ClassDoc> list, boolean isEnum, Content contentTree) {
    int size = list.size();
    if (size > 0) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        addLevelInfo(local, classtree.subs(local, isEnum), isEnum, li); // Recurse
        ul.addContent(li);
      }
  ...
  protected void addTree(List<ClassDoc> list, String heading, Content div) {
    if (list.size() > 0) {
      ...
      addLevelInfo(
          !firstClassDoc.isInterface() ? firstClassDoc : null,
          list,
    ...
    if (interfaces.length > (cd.isInterface() ? 1 : 0)) {
      ...
      for (int i = 0; i < interfaces.length; i++) {
        if (parent != interfaces[i]) {
          if (!(interfaces[i].isPublic() || Util.isLinkable(interfaces[i], configuration))) {
            continue;
          }
          if (counter == 0) {
            if (cd.isInterface()) {
          ...
          addPreQualifiedClassLink(LinkInfoImpl.Kind.TREE, interfaces[i], contentTree);
          counter++;
        }
      }
      if (counter > 0) {
