...
public class PackageIndexFrameWriter extends AbstractPackageIndexWriter {
  ...
  protected void addPackagesList(
    ...
    Content heading = HtmlTree.HEADING(HtmlConstants.PACKAGE_HEADING, true, packagesLabel);
  ...
  protected Content getPackage(PackageDoc pd) {
    ...
    if (pd.name().length() > 0) {
      ...
      packageLinkContent =
          getHyperLink(pathString(pd, DocPaths.PACKAGE_FRAME), packageLabel, "", "packageFrame");
    } else {
  ...
  protected void addAllProfilesLink(Content div) {
    Content linkContent =
        getHyperLink(DocPaths.PROFILE_OVERVIEW_FRAME, allprofilesLabel, "", "packageListFrame");
    Content span = HtmlTree.SPAN(linkContent);
    div.addContent(span);
  }
