...
public class HtmlDocletWriter extends HtmlDocWriter {
  ...
  public Content getTargetProfilePackageLink(
      PackageDoc pd, String target, Content label, String profileName) {
    return getHyperLink(
        pathString(pd, DocPaths.profilePackageSummary(profileName)), label, "", target);
  }
   ...
   */
  public Content getTargetProfileLink(String target, Content label, String profileName) {
    return getHyperLink(
        pathToRoot.resolve(DocPaths.profileSummary(profileName)), label, "", target);
  }
  ...
  public String getTypeNameForProfile(ClassDoc cd) {
    StringBuilder typeName = new StringBuilder((cd.containingPackage()).name().replace(".", "/"));
    typeName.append("/").append(cd.name().replace(".", "$"));
  ...
  public void addClassesSummary(
    ...
    if (classes.length > 0) {
      ...
      Content table = HtmlTree.TABLE(HtmlStyle.typeSummary, 0, 3, 0, tableSummary, caption);
      ...
      for (int i = 0; i < classes.length; i++) {
        ...
        if (!Util.isCoreClass(classes[i]) || !configuration.isGeneratedDoc(classes[i])) {
          continue;
        }
        Content classContent =
            getLink(new LinkInfoImpl(configuration, LinkInfoImpl.Kind.PACKAGE, classes[i]));
        ...
        if (Util.isDeprecated(classes[i])) {
          ...
          if (classes[i].tags("deprecated").length > 0) {
            addSummaryDeprecatedComment(
                classes[i], classes[i].tags("deprecated")[0], tdClassDescription);
          }
        } else addSummaryComment(classes[i], tdClassDescription);
  ...
  protected void addNavLinks(boolean header, Content body) {
    if (!configuration.nonavbar) {
      ...
      if (header) {
        ...
        Content skipLinkContent =
            HtmlTree.DIV(
                HtmlStyle.skipNav,
                getHyperLink(
                    getDocLink(SectionName.SKIP_NAVBAR_TOP),
                    skipNavLinks,
        ...
        Content skipLinkContent =
            HtmlTree.DIV(
                HtmlStyle.skipNav,
                getHyperLink(
                    getDocLink(SectionName.SKIP_NAVBAR_BOTTOM),
                    skipNavLinks,
                    skipNavLinks.toString(),
                    ""));
        navDiv.addContent(skipLinkContent);
      ...
      navList.addAttr(HtmlAttr.TITLE, configuration.getText("doclet.Navigation"));
  ...
  protected Content getNavLinkContents() {
    Content linkContent =
        getHyperLink(pathToRoot.resolve(DocPaths.OVERVIEW_SUMMARY), overviewLabel, "", "");
  ...
  protected Content getNavLinkTree() {
    ...
    if (packages.length == 1 && configuration.root.specifiedClasses().length == 0) {
      treeLinkContent =
          getHyperLink(pathString(packages[0], DocPaths.PACKAGE_TREE), treeLabel, "", "");
    } else {
      treeLinkContent = getHyperLink(pathToRoot.resolve(DocPaths.OVERVIEW_TREE), treeLabel, "", "");
    }
  ...
  protected Content getNavLinkMainTree(String label) {
    Content mainTreeContent =
        getHyperLink(pathToRoot.resolve(DocPaths.OVERVIEW_TREE), new StringContent(label));
  ...
  protected Content getNavLinkDeprecated() {
    Content linkContent =
        getHyperLink(pathToRoot.resolve(DocPaths.DEPRECATED_LIST), deprecatedLabel, "", "");
    Content li = HtmlTree.LI(linkContent);
    return li;
  }
  ...
  protected Content getNavLinkClassIndex() {
    Content allClassesContent =
        getHyperLink(pathToRoot.resolve(DocPaths.ALLCLASSES_NOFRAME), allclassesLabel, "", "");
  ...
  protected Content getNavLinkHelp() {
    ...
    Content linkContent = getHyperLink(pathToRoot.resolve(helpfilenm), helpLabel, "", "");
    Content li = HtmlTree.LI(linkContent);
    return li;
  }
  ...
  public Content getSummaryTableHeader(String[] header, String scope) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (i == 0) tr.addContent(HtmlTree.TH(HtmlStyle.colFirst, scope, tableHeader));
      else if (i == (size - 1)) tr.addContent(HtmlTree.TH(HtmlStyle.colLast, scope, tableHeader));
      else tr.addContent(HtmlTree.TH(scope, tableHeader));
    }
    return tr;
  }
  ...
  public Content getMarkerAnchor(String anchorName, Content anchorContent) {
    if (anchorContent == null) anchorContent = new Comment(" ");
   ...
   */
  protected void addPackageDeprecatedAPI(
      List<Doc> deprPkgs,
      String headingKey,
      String tableSummary,
      String[] tableHeader,
    ...
    if (deprPkgs.size() > 0) {
      ...
      for (int i = 0; i < deprPkgs.size(); i++) {
        ...
        HtmlTree td = HtmlTree.TD(HtmlStyle.colOne, getPackageLink(pkg, getPackageName(pkg)));
  ...
  public Content getPackageLink(PackageDoc pkg, Content label) {
    ...
    if (included || pkg == null) {
      return getHyperLink(pathString(pkg, DocPaths.PACKAGE_SUMMARY), label);
    } else {
  ...
  public void addSrcLink(ProgramElementDoc doc, Content label, Content htmltree) {
    ...
    DocPath href = pathToRoot.resolve(DocPaths.SOURCE_OUTPUT).resolve(DocPath.forClass(cd));
    Content linkContent =
        getHyperLink(href.fragment(SourceToHTMLConverter.getAnchorName(doc)), label, "", "");
      ...
      if (getCrossPackageLink(packageName) != null) {
        ...
        DocLink link =
            configuration.extern.getExternalLink(
                packageName, pathToRoot, className + ".html", refMemName);
        return getHyperLink(
            link,
  ...
  public DocLink getCrossPackageLink(String pkgName) {
    return configuration.extern.getExternalLink(
        pkgName, pathToRoot, DocPaths.PACKAGE_SUMMARY.getPath());
  }
  ...
  public Content getQualifiedClassLink(LinkInfoImpl.Kind context, ClassDoc cd) {
    return getLink(
        new LinkInfoImpl(configuration, context, cd).label(configuration.getClassName(cd)));
  }
  ...
  public void addPreQualifiedClassLink(
      LinkInfoImpl.Kind context, ClassDoc cd, Content contentTree) {
   ...
   */
  public Content getPreQualifiedClassLink(
      LinkInfoImpl.Kind context, ClassDoc cd, boolean isStrong) {
    ContentBuilder classlink = new ContentBuilder();
    PackageDoc pd = cd.containingPackage();
    if (pd != null && !configuration.shouldExcludeQualifier(pd.name())) {
      classlink.addContent(getPkgName(cd));
    }
    classlink.addContent(
        getLink(new LinkInfoImpl(configuration, context, cd).label(cd.name()).strong(isStrong)));
    return classlink;
  }
   ...
   */
  public void addPreQualifiedClassLink(
      LinkInfoImpl.Kind context, ClassDoc cd, boolean isStrong, Content contentTree) {
    PackageDoc pd = cd.containingPackage();
    if (pd != null && !configuration.shouldExcludeQualifier(pd.name())) {
  ...
  public Content getDocLink(LinkInfoImpl.Kind context, MemberDoc doc, String label) {
    return getDocLink(context, doc.containingClass(), doc, new StringContent(label));
  }
  ...
  public Content getDocLink(
      LinkInfoImpl.Kind context, MemberDoc doc, String label, boolean strong) {
   ...
   */
  public Content getDocLink(
      LinkInfoImpl.Kind context, ClassDoc classDoc, MemberDoc doc, String label, boolean strong) {
    return getDocLink(context, classDoc, doc, label, strong, false);
  }

  public Content getDocLink(
      LinkInfoImpl.Kind context, ClassDoc classDoc, MemberDoc doc, Content label, boolean strong) {
    return getDocLink(context, classDoc, doc, label, strong, false);
  }
  ...
  public Content getDocLink(
      ...
      boolean strong,
      boolean isProperty) {
    if (!(doc.isIncluded() || Util.isLinkable(classDoc, configuration))) {
      return label;
    } else if (doc instanceof ExecutableMemberDoc) {
      ExecutableMemberDoc emd = (ExecutableMemberDoc) doc;
      return getLink(
          new LinkInfoImpl(configuration, context, classDoc)
              .label(label)
              .where(getName(getAnchor(emd, isProperty)))
              .strong(strong));
    } else if (doc instanceof MemberDoc) {
      return getLink(
          new LinkInfoImpl(configuration, context, classDoc)
              .label(label)
              .where(getName(doc.name()))
              .strong(strong));
    } else {
      return label;
    }
   ...
   */
  public Content getDocLink(
      LinkInfoImpl.Kind context, ClassDoc classDoc, MemberDoc doc, Content label) {
    if (!(doc.isIncluded() || Util.isLinkable(classDoc, configuration))) {
      return label;
    } else if (doc instanceof ExecutableMemberDoc) {
      ExecutableMemberDoc emd = (ExecutableMemberDoc) doc;
  ...
  public Content seeTagToContent(SeeTag see) {
    ...
    if (refClass == null) {
      ...
      if (refPackage != null && refPackage.isIncluded()) {
        ...
        if (label.isEmpty()) label = plainOrCode(plain, new StringContent(refPackage.name()));
        ...
        if (packageCrossLink != null) {
          ...
          return getHyperLink(packageCrossLink, (label.isEmpty() ? text : label));
        } else if ((classCrossLink =
                getCrossClassLink(refClassName, refMemName, label, false, "", !plain))
            != null) {
      ...
      return getLink(
          new LinkInfoImpl(configuration, LinkInfoImpl.Kind.DEFAULT, refClass).label(label));
    } else if (refMem == null) {
      ...
      if (see.text().trim().startsWith("#")
          && !(containing.isPublic() || Util.isLinkable(containing, configuration))) {
      ...
      return getDocLink(
          LinkInfoImpl.Kind.SEE_TAG, containing, refMem, (label.isEmpty() ? text : label), false);
    }
  }
  ...
  private void addCommentTags(
      Doc doc, Tag holderTag, Tag[] tags, boolean depr, boolean first, Content htmltree) {
    if (configuration.nocomment) {
      return;
    }
    Content div;
  ...
  public Content commentTagsToContent(Tag holderTag, Doc doc, Tag[] tags, boolean isFirstSentence) {
    ...
    for (int i = 0; i < tags.length; i++) {
      ...
      if (tagelem instanceof SeeTag) {
        ...
        if (output != null) result.addContent(output);
        if (wasEmpty
            && isFirstSentence
            && tagelem.name().equals("@inheritDoc")
            && !result.isEmpty()) {
          break;
        } else {
          continue;
      ...
      while (begin >= 0) {
        ...
        if (begin == 0) {
          ...
          configuration.root.printWarning(
              doc.position(), configuration.getText("doclet.malformed_html_link_tag", text));
          break;
        }
        if (end == -1) {
  ...
  static {
    for (HtmlTag t : HtmlTag.values()) {
      if (t.blockType == HtmlTag.BlockType.BLOCK) blockTags.add(t.value);
    }
  }
  ...
  private static boolean isHtmlTagLetterOrDigit(char ch) {
    return ('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z') || ('1' <= ch && ch <= '6');
  }
  ...
  public HtmlTree getScriptProperties() {
    HtmlTree script =
        HtmlTree.SCRIPT("text/javascript", pathToRoot.resolve(DocPaths.JAVASCRIPT).getPath());
    return script;
  }
  ...
  public void addReceiverAnnotationInfo(
      ExecutableMemberDoc method, AnnotationDesc[] descList, Content htmltree) {
   ...
   *        added
   */
  private boolean addAnnotationInfo(
      int indent, Doc doc, AnnotationDesc[] descList, boolean lineBreak, Content htmltree) {
  ...
  public List<Content> getAnnotations(
    ...
    for (int i = 0; i < descList.length; i++) {
      ...
      LinkInfoImpl linkInfo =
          new LinkInfoImpl(configuration, LinkInfoImpl.Kind.ANNOTATION, annotationDoc);
      ...
      if (descList[i].isSynthesized()) {
        ...
        if (pairs.length == 1 && isAnnotationDocumented) {
          AnnotationValue[] annotationArray = (AnnotationValue[]) (pairs[0].value()).value();
          List<AnnotationValue> annotationTypeValues = new ArrayList<AnnotationValue>();
          annotationTypeValues.addAll(Arrays.asList(annotationArray));
          String sep = "";
          for (AnnotationValue av : annotationTypeValues) {
        ...
        // repeatable type annotation is not documented, print the container.
          ...
          addAnnotations(annotationDoc, linkInfo, annotation, pairs, indent, false);
        }
      } else {
        addAnnotations(annotationDoc, linkInfo, annotation, pairs, indent, linkBreak);
      }
  ...
  private void addAnnotations(
    ...
    if (pairs.length > 0) {
      ...
      for (int j = 0; j < pairs.length; j++) {
        ...
        annotation.addContent(
            getDocLink(
                LinkInfoImpl.Kind.ANNOTATION,
                pairs[j].element(),
                pairs[j].element().name(),
                false));
        annotation.addContent("=");
        AnnotationValue annotationValue = pairs[j].value();
        List<AnnotationValue> annotationTypeValues = new ArrayList<AnnotationValue>();
  ...
  private Content annotationValueToContent(AnnotationValue annotationValue) {
    if (annotationValue.value() instanceof Type) {
      ...
      if (type.asClassDoc() != null) {
        LinkInfoImpl linkInfo = new LinkInfoImpl(configuration, LinkInfoImpl.Kind.ANNOTATION, type);
        linkInfo.label =
            new StringContent(
                (type.asClassDoc().isIncluded() ? type.typeName() : type.qualifiedTypeName())
                    + type.dimension()
                    + ".class");
      ...
      List<Content> list =
          getAnnotations(0, new AnnotationDesc[] {(AnnotationDesc) annotationValue.value()}, false);
      ...
      return getDocLink(
          ...
          ((MemberDoc) annotationValue.value()).name(),
          false);
    } else {
