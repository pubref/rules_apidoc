...
public abstract class AbstractMemberWriter {
  ...
  protected void addUseInfo(
    ...
    if (members.size() > 0) {
      Content table =
          HtmlTree.TABLE(
              HtmlStyle.useSummary, 0, 3, 0, tableSummary, writer.getTableCaption(heading));
      Content tbody = new HtmlTree(HtmlTag.TBODY);
  ...
  public Content getMemberTree(Content memberTree, boolean isLastContent) {
    if (isLastContent) return HtmlTree.UL(HtmlStyle.blockListLast, memberTree);
    else return HtmlTree.UL(HtmlStyle.blockList, memberTree);
  }
}
