...
public class ClassWriterImpl extends SubWriterHolderWriter implements ClassWriter {
  ...
  public Content getNavLinkPrevious() {
    ...
    if (prev != null) {
      Content prevLink =
          getLink(
              new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS, prev)
  ...
  public Content getNavLinkNext() {
    ...
    if (next != null) {
      Content nextLink =
          getLink(
              new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS, next)
  ...
  public Content getHeader(String header) {
    String pkgname =
        (classDoc.containingPackage() != null) ? classDoc.containingPackage().name() : "";
    ...
    LinkInfoImpl linkInfo =
        new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS_HEADER, classDoc);
  ...
  public void printDocument(Content contentTree) throws IOException {
    printHtmlDocument(configuration.metakeywords.getMetaKeywords(classDoc), true, contentTree);
  }
  ...
  public void addClassSignature(String modifiers, Content classInfoTree) {
    ...
    LinkInfoImpl linkInfo =
        new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS_SIGNATURE, classDoc);
    ...
    if (!isInterface) {
      Type superclass = Util.getFirstVisibleSuperClass(classDoc, configuration);
      if (superclass != null) {
        ...
        Content link =
            getLink(
                new LinkInfoImpl(
                    configuration, LinkInfoImpl.Kind.CLASS_SIGNATURE_PARENT_NAME, superclass));
        pre.addContent(link);
      }
    }
    ...
    if (implIntfacs != null && implIntfacs.length > 0) {
      ...
      for (int i = 0; i < implIntfacs.length; i++) {
        ...
        Content link =
            getLink(
                new LinkInfoImpl(
                    configuration, LinkInfoImpl.Kind.CLASS_SIGNATURE_PARENT_NAME, implIntfacs[i]));
        pre.addContent(link);
        counter++;
      }
    }
    ...
    do {
      sup =
          Util.getFirstVisibleSuperClass(
              type instanceof ClassDoc ? (ClassDoc) type : type.asClassDoc(), configuration);
      if (sup != null) {
        HtmlTree ul = new HtmlTree(HtmlTag.UL);
        ul.addStyle(HtmlStyle.inheritance);
        ul.addContent(getTreeForClassHelper(type));
        if (liTree != null) ul.addContent(liTree);
        Content li = HtmlTree.LI(ul);
        liTree = li;
        type = sup;
      } else classTreeUl.addContent(getTreeForClassHelper(type));
    } while (sup != null);
    if (liTree != null) classTreeUl.addContent(liTree);
    return classTreeUl;
  }
  ...
  private Content getTreeForClassHelper(Type type) {
    ...
    if (type.equals(classDoc)) {
      Content typeParameters =
          getTypeParameterLinks(new LinkInfoImpl(configuration, LinkInfoImpl.Kind.TREE, classDoc));
      ...
      Content link =
          getLink(
              new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS_TREE_PARENT, type)
  ...
  public void addTypeParamInfo(Content classInfoTree) {
    if (classDoc.typeParamTags().length > 0) {
      Content typeParam =
          (new ParamTaglet()).getTagletOutput(classDoc, getTagletWriterInstance(false));
  ...
  public void addSubClassInfo(Content classInfoTree) {
    if (classDoc.isClass()) {
      ...
      if (subclasses.size() > 0) {
        Content label = getResource("doclet.Subclasses");
        Content dt = HtmlTree.DT(label);
        Content dl = HtmlTree.DL(dt);
        dl.addContent(getClassLinks(LinkInfoImpl.Kind.SUBCLASSES, subclasses));
        classInfoTree.addContent(dl);
      }
    }
  }
  ...
  public void addSubInterfacesInfo(Content classInfoTree) {
    if (classDoc.isInterface()) {
      ...
      if (subInterfaces.size() > 0) {
        Content label = getResource("doclet.Subinterfaces");
        Content dt = HtmlTree.DT(label);
        Content dl = HtmlTree.DL(dt);
        dl.addContent(getClassLinks(LinkInfoImpl.Kind.SUBINTERFACES, subInterfaces));
        classInfoTree.addContent(dl);
      }
    }
  }
  ...
  public void addInterfaceUsageInfo(Content classInfoTree) {
    ...
    if (implcl.size() > 0) {
      Content label = getResource("doclet.Implementing_Classes");
      Content dt = HtmlTree.DT(label);
      Content dl = HtmlTree.DL(dt);
      dl.addContent(getClassLinks(LinkInfoImpl.Kind.IMPLEMENTED_CLASSES, implcl));
      classInfoTree.addContent(dl);
    }
  }
  ...
  public void addImplementedInterfacesInfo(Content classInfoTree) {
    ...
    if (classDoc.isClass() && interfaceArray.size() > 0) {
      Content label = getResource("doclet.All_Implemented_Interfaces");
      Content dt = HtmlTree.DT(label);
      Content dl = HtmlTree.DL(dt);
      dl.addContent(getClassLinks(LinkInfoImpl.Kind.IMPLEMENTED_INTERFACES, interfaceArray));
      classInfoTree.addContent(dl);
    }
  }
  ...
  public void addSuperInterfacesInfo(Content classInfoTree) {
    ...
    if (classDoc.isInterface() && interfaceArray.size() > 0) {
      ...
      Content dt = HtmlTree.DT(label);
      Content dl = HtmlTree.DL(dt);
      dl.addContent(getClassLinks(LinkInfoImpl.Kind.SUPER_INTERFACES, interfaceArray));
      classInfoTree.addContent(dl);
    }
  }
  ...
  public void addNestedClassInfo(Content classInfoTree) {
    ...
    if (outerClass != null) {
      ...
      if (outerClass.isInterface()) {
        label = getResource("doclet.Enclosing_Interface");
      } else {
        label = getResource("doclet.Enclosing_Class");
      }
      Content dt = HtmlTree.DT(label);
      Content dl = HtmlTree.DL(dt);
      Content dd = new HtmlTree(HtmlTag.DD);
      dd.addContent(getLink(new LinkInfoImpl(configuration, LinkInfoImpl.Kind.CLASS, outerClass)));
      dl.addContent(dd);
      classInfoTree.addContent(dl);
    }
  }
  ...
  private Content getClassLinks(LinkInfoImpl.Kind context, List<?> list) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (typeList[i] instanceof ClassDoc) {
        Content link = getLink(new LinkInfoImpl(configuration, context, (ClassDoc) (typeList[i])));
        dd.addContent(link);
      } else {
        Content link = getLink(new LinkInfoImpl(configuration, context, (Type) (typeList[i])));
        dd.addContent(link);
      }
    }
    return dd;
  ...
  protected Content getNavLinkTree() {
    Content treeLinkContent = getHyperLink(DocPaths.PACKAGE_TREE, treeLabel, "", "");
  ...
  protected Content getNavSummaryLinks() throws Exception {
    ...
    for (int i = 0; i < navLinkLabels.length; i++) {
      ...
      AbstractMemberWriter writer =
          ((AbstractMemberWriter) memberSummaryBuilder.getMemberSummaryWriter(i));
      if (writer == null) {
        liNav.addContent(getResource(navLinkLabels[i]));
      } else {
        writer.addNavSummaryLink(
            memberSummaryBuilder.members(i), memberSummaryBuilder.getVisibleMemberMap(i), liNav);
      }
