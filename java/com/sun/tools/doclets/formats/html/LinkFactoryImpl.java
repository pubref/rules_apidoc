...
public class LinkFactoryImpl extends LinkFactory {
  ...
  protected Content getClassLink(LinkInfo linkInfo) {
    ...
    if (classDoc.isIncluded()) {
      if (configuration.isGeneratedDoc(classDoc)) {
        ...
        if (linkInfo.linkToSelf || !(DocPath.forName(classDoc)).equals(m_writer.filename)) {
          link.addContent(
              m_writer.getHyperLink(
  ...
  protected Content getTypeParameterLink(LinkInfo linkInfo, Type typeParam) {
    LinkInfoImpl typeLinkInfo =
        new LinkInfoImpl(m_writer.configuration, ((LinkInfoImpl) linkInfo).getContext(), typeParam);
  ...
  public Content getTypeAnnotationLinks(LinkInfo linkInfo) {
    ...
    if (annotations.length == 0) return links;

    List<Content> annos =
        m_writer.getAnnotations(0, annotations, false, linkInfo.isJava5DeclarationLocation);
  ...
  private String getClassToolTip(ClassDoc classDoc, boolean isTypeLink) {
    ...
    if (isTypeLink) {
      ...
      return configuration.getText(
          "doclet.Href_Interface_Title", Util.getPackageName(classDoc.containingPackage()));
    } else if (classDoc.isAnnotationType()) {
      return configuration.getText(
          "doclet.Href_Annotation_Title", Util.getPackageName(classDoc.containingPackage()));
    } else if (classDoc.isEnum()) {
      return configuration.getText(
          "doclet.Href_Enum_Title", Util.getPackageName(classDoc.containingPackage()));
    } else {
      return configuration.getText(
          "doclet.Href_Class_Title", Util.getPackageName(classDoc.containingPackage()));
    }
  }
