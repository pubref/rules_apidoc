...
public class PropertyWriterImpl extends AbstractMemberWriter
  ...
  public Content getPropertyDetailsTreeHeader(ClassDoc classDoc, Content memberDetailsTree) {
    ...
    propertyDetailsTree.addContent(writer.getMarkerAnchor(SectionName.PROPERTY_DETAIL));
    Content heading = HtmlTree.HEADING(HtmlConstants.DETAILS_HEADING, writer.propertyDetailsLabel);
  ...
  public Content getPropertyDocTreeHeader(MethodDoc property, Content propertyDetailsTree) {
    propertyDetailsTree.addContent(writer.getMarkerAnchor(property.name()));
  ...
  public Content getSignature(MethodDoc property) {
    ...
    Content propertylink =
        writer.getLink(
            new LinkInfoImpl(configuration, LinkInfoImpl.Kind.MEMBER, property.returnType()));
  ...
  public void addComments(MethodDoc property, Content propertyDocTree) {
    ...
    if (property.inlineTags().length > 0) {
      if (holder.equals(classdoc)
        ...
        Content link =
            writer.getDocLink(
                LinkInfoImpl.Kind.PROPERTY_DOC_COPY,
                holder,
                property,
  ...
  public void addSummaryLabel(Content memberTree) {
    Content label =
        HtmlTree.HEADING(
            HtmlConstants.SUMMARY_HEADING, writer.getResource("doclet.Property_Summary"));
  ...
  public void addSummaryAnchor(ClassDoc cd, Content memberTree) {
    memberTree.addContent(writer.getMarkerAnchor(SectionName.PROPERTY_SUMMARY));
  }
  ...
  public void addInheritedSummaryAnchor(ClassDoc cd, Content inheritedTree) {
    inheritedTree.addContent(
        writer.getMarkerAnchor(SectionName.PROPERTIES_INHERITANCE, configuration.getClassName(cd)));
  }
  ...
  protected Content getDeprecatedLink(ProgramElementDoc member) {
    return writer.getDocLink(
        LinkInfoImpl.Kind.MEMBER, (MemberDoc) member, ((MethodDoc) member).qualifiedName());
  }
    ...
    if (link) {
      if (cd == null) {
        return writer.getHyperLink(
            SectionName.PROPERTY_SUMMARY, writer.getResource("doclet.navProperty"));
      } else {
        return writer.getHyperLink(
  ...
  protected void addNavDetailLink(boolean link, Content liNav) {
    if (link) {
      liNav.addContent(
          writer.getHyperLink(
              SectionName.PROPERTY_DETAIL, writer.getResource("doclet.navProperty")));
    } else {
