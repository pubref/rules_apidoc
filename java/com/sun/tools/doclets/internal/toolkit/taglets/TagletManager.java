...
public class TagletManager {
  ...
  public void addCustomTag(String classname, JavaFileManager fileManager, String tagletPath) {
    try {
      ...
      Method meth = customTagClass.getMethod("register", new Class<?>[] {java.util.Map.class});
      ...
      Object newLastTag = (list != null && list.length > 0) ? list[list.length - 1] : null;
  ...
  private void initCustomTagletArrays() {
    ...
    if (!nosince) sTags.add(customTags.get("since"));
  ...
  private void initStandardTaglets() {
    ...
    addStandardTaglet(
        new SimpleTaglet("exception", null, SimpleTaglet.METHOD + SimpleTaglet.CONSTRUCTOR));
    addStandardTaglet(
        !nosince, new SimpleTaglet("since", message.getText("doclet.Since"), SimpleTaglet.ALL));
    addStandardTaglet(
        showversion,
        new SimpleTaglet(
            "version",
            message.getText("doclet.Version"),
            SimpleTaglet.PACKAGE + SimpleTaglet.TYPE + SimpleTaglet.OVERVIEW));
    addStandardTaglet(
        showauthor,
        new SimpleTaglet(
            "author",
            message.getText("doclet.Author"),
            SimpleTaglet.PACKAGE + SimpleTaglet.TYPE + SimpleTaglet.OVERVIEW));
    addStandardTaglet(
        new SimpleTaglet(
            "serialData", message.getText("doclet.SerialData"), SimpleTaglet.EXCLUDED));
    customTags.put(
        (temp = new SimpleTaglet("factory", message.getText("doclet.Factory"), SimpleTaglet.METHOD))
            .getName(),
        temp);
  ...
  private void initJavaFXTaglets() {
    ...
    addStandardTaglet(
        new SimpleTaglet(
            "treatAsPrivate", null, SimpleTaglet.FIELD + SimpleTaglet.METHOD + SimpleTaglet.TYPE));
  }
  ...
  void addStandardTaglet(boolean enable, Taglet taglet) {
    String name = taglet.getName();
    if (enable) customTags.put(name, taglet);
    standardTags.add(name);
  }
