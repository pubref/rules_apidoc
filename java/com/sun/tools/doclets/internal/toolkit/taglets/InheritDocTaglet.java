...
public class InheritDocTaglet extends BaseInlineTaglet {
  ...
  private Content retrieveInheritedDocumentation(
      TagletWriter writer, ProgramElementDoc ped, Tag holderTag, boolean isFirstSentence) {
    ...
    if (inheritableTaglet != null && !(inheritableTaglet instanceof InheritableTaglet)) {
      ...
      configuration.message.warning(ped.position(), "doclet.noInheritedDoc", message);
    }
    DocFinder.Output inheritedDoc =
        DocFinder.search(
            new DocFinder.Input(
                ped, (InheritableTaglet) inheritableTaglet, holderTag, isFirstSentence, true));
  ...
  public Content getTagletOutput(Tag tag, TagletWriter tagletWriter) {
    ...
    return tag.name().equals("@inheritDoc")
        ? retrieveInheritedDocumentation(
            tagletWriter, (ProgramElementDoc) tag.holder(), null, tagletWriter.isFirstSentence)
        : retrieveInheritedDocumentation(
            tagletWriter, (ProgramElementDoc) tag.holder(), tag, tagletWriter.isFirstSentence);
  }
}
