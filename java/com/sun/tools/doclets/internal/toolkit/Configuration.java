...
public abstract class Configuration {
  ...
  public Configuration() {
    message =
        new MessageRetriever(this, "com.sun.tools.doclets.internal.toolkit.resources.doclets");
   ...
   */
  public abstract boolean validOptions(String options[][], DocErrorReporter reporter);

  private void initProfiles() throws IOException {
    if (profilespath.isEmpty()) return;
    ...
    for (Profile p : Profile.values()) interimResults.put(p, new ArrayList<PackageDoc>());
    ...
    for (Map.Entry<Profile, List<PackageDoc>> e : interimResults.entrySet()) {
      ...
      if (size > 0) profilePackages.put(p.name, pkgs.toArray(new PackageDoc[pkgs.size()]));
      prev = pkgs;
    }
  ...
  public void setOptions(String[][] options) throws Fault {
    ...
    for (int oi = 0; oi < options.length; ++oi) {
      ...
      if (opt.equals("-docfilessubdirs")) {
        ...
        sourcepath = os[1];
      } else if ((opt.equals("-classpath") || opt.equals("-cp")) && sourcepath.length() == 0) {
        sourcepath = os[1];
  ...
  private void ensureOutputDirExists() throws Fault {
    ...
    if (!destDir.exists()) {
      ...
      throw new Fault(getText("doclet.destination_directory_not_directory_0", destDir.getPath()));
    } else if (!destDir.canWrite()) {
      throw new Fault(getText("doclet.destination_directory_not_writable_0", destDir.getPath()));
    }
  }
  ...
  private void initTagletManager(Set<String[]> customTagStrs) {
    ...
    for (Iterator<String[]> it = customTagStrs.iterator(); it.hasNext(); ) {
      ...
      String[] tokens = tokenize(args[1], TagletManager.SIMPLE_TAGLET_OPT_SEPARATOR, 3);
      if (tokens.length == 1) {
