...
public class ProfilePackageSummaryBuilder extends AbstractBuilder {
   ...
   */
  private ProfilePackageSummaryBuilder(
      Context context,
      PackageDoc pkg,
      ProfilePackageSummaryWriter profilePackageWriter,
      Profile profile) {
    super(context);
    this.packageDoc = pkg;
   ...
   */
  public static ProfilePackageSummaryBuilder getInstance(
      Context context,
      PackageDoc pkg,
      ProfilePackageSummaryWriter profilePackageWriter,
      Profile profile) {
  ...
  public void buildPackageDoc(XMLNode node, Content contentTree) throws Exception {
    contentTree = profilePackageWriter.getPackageHeader(Util.getPackageName(packageDoc));
  ...
  public void buildInterfaceSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] interfaceTableHeader =
        new String[] {
          configuration.getText("doclet.Interface"), configuration.getText("doclet.Description")
        };
    ClassDoc[] interfaces =
  ...
  public void buildClassSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] classTableHeader =
        new String[] {
          configuration.getText("doclet.Class"), configuration.getText("doclet.Description")
        };
    ClassDoc[] classes =
        packageDoc.isIncluded()
  ...
  public void buildEnumSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] enumTableHeader =
        new String[] {
          configuration.getText("doclet.Enum"), configuration.getText("doclet.Description")
        };
    ClassDoc[] enums =
        packageDoc.isIncluded()
            ? packageDoc.enums()
            : configuration.classDocCatalog.enums(Util.getPackageName(packageDoc));
    if (enums.length > 0) {
      profilePackageWriter.addClassesSummary(
          enums,
          configuration.getText("doclet.Enum_Summary"),
  ...
  public void buildExceptionSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] exceptionTableHeader =
        new String[] {
          configuration.getText("doclet.Exception"), configuration.getText("doclet.Description")
        };
    ClassDoc[] exceptions =
        packageDoc.isIncluded()
  ...
  public void buildErrorSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] errorTableHeader =
        new String[] {
          configuration.getText("doclet.Error"), configuration.getText("doclet.Description")
        };
    ClassDoc[] errors =
        packageDoc.isIncluded()
            ? packageDoc.errors()
            : configuration.classDocCatalog.errors(Util.getPackageName(packageDoc));
    if (errors.length > 0) {
      profilePackageWriter.addClassesSummary(
          errors,
          configuration.getText("doclet.Error_Summary"),
  ...
  public void buildAnnotationTypeSummary(XMLNode node, Content summaryContentTree) {
    ...
    String[] annotationtypeTableHeader =
        new String[] {
