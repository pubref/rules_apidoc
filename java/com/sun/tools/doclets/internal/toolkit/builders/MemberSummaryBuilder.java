...
public class MemberSummaryBuilder extends AbstractMemberBuilder {
  ...
  private MemberSummaryBuilder(Context context, ClassDoc classDoc) {
    ...
    for (int i = 0; i < VisibleMemberMap.NUM_MEMBER_TYPES; i++) {
      visibleMemberMaps[i] = new VisibleMemberMap(classDoc, i, configuration);
    }
  }
  ...
  public static MemberSummaryBuilder getInstance(ClassWriter classWriter, Context context)
    ...
    builder.memberSummaryWriters = new MemberSummaryWriter[VisibleMemberMap.NUM_MEMBER_TYPES];
  ...
  public static MemberSummaryBuilder getInstance(
      AnnotationTypeWriter annotationTypeWriter, Context context) throws Exception {
    MemberSummaryBuilder builder =
        new MemberSummaryBuilder(context, annotationTypeWriter.getAnnotationTypeDoc());
    builder.memberSummaryWriters = new MemberSummaryWriter[VisibleMemberMap.NUM_MEMBER_TYPES];
    WriterFactory wf = context.configuration.getWriterFactory();
    for (int i = 0; i < VisibleMemberMap.NUM_MEMBER_TYPES; i++) {
      builder.memberSummaryWriters[i] =
          builder.visibleMemberMaps[i].noVisibleMembers()
              ? null
              : wf.getMemberSummaryWriter(annotationTypeWriter, i);
    }
    return builder;
  }
  ...
  public void buildEnumConstantsSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.ENUM_CONSTANTS];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.ENUM_CONSTANTS];
  ...
  public void buildAnnotationTypeFieldsSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.ANNOTATION_TYPE_FIELDS];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.ANNOTATION_TYPE_FIELDS];
    addSummary(writer, visibleMemberMap, false, memberSummaryTree);
  }
  ...
  public void buildFieldsSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.FIELDS];
  ...
  public void buildPropertiesSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.PROPERTIES];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.PROPERTIES];
    addSummary(writer, visibleMemberMap, true, memberSummaryTree);
  }
  ...
  public void buildNestedClassesSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.INNERCLASSES];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.INNERCLASSES];
    addSummary(writer, visibleMemberMap, true, memberSummaryTree);
  }
  ...
  public void buildMethodsSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.METHODS];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.METHODS];
    addSummary(writer, visibleMemberMap, true, memberSummaryTree);
  }
  ...
  public void buildConstructorsSummary(XMLNode node, Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.CONSTRUCTORS];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.CONSTRUCTORS];
    addSummary(writer, visibleMemberMap, false, memberSummaryTree);
  }
  ...
  private void buildSummary(
    ...
    List<ProgramElementDoc> members =
        new ArrayList<ProgramElementDoc>(visibleMemberMap.getLeafClassMembers(configuration));
    if (members.size() > 0) {
      Collections.sort(members);
      List<Content> tableContents = new LinkedList<Content>();
      for (int i = 0; i < members.size(); i++) {
        ProgramElementDoc member = members.get(i);
        final ProgramElementDoc propertyDoc = visibleMemberMap.getPropertyMemberDoc(member);
        if (propertyDoc != null) {
        ...
        writer.addMemberSummary(classDoc, member, firstSentenceTags, tableContents, i);
      }
  ...
  private void processProperty(
      VisibleMemberMap visibleMemberMap, ProgramElementDoc member, ProgramElementDoc propertyDoc) {
      ...
      if (inhmembers.size() > 0) {
        ...
        for (int j = 0; j < inhmembers.size(); ++j) {
          writer.addInheritedMemberSummary(
              inhclass.isPackagePrivate() && !Util.isLinkable(inhclass, configuration)
                  ? classDoc
                  : inhclass,
              inhmembers.get(j),
              j == 0,
  ...
  private void addSummary(
    ...
    if (showInheritedSummary) buildInheritedSummary(writer, visibleMemberMap, summaryTreeList);
    if (!summaryTreeList.isEmpty()) {
      Content memberTree = writer.getMemberSummaryHeader(classDoc, memberSummaryTree);
