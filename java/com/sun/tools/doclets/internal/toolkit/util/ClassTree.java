...
public class ClassTree {
  ...
  private Map<ClassDoc, List<ClassDoc>> subAnnotationTypes =
      new HashMap<ClassDoc, List<ClassDoc>>();

  /**
   * Mapping for each Interface with classes who implement it.
   */
  private Map<ClassDoc, List<ClassDoc>> implementingclasses =
      new HashMap<ClassDoc, List<ClassDoc>>();
  ...
  private void buildTree(ClassDoc[] classes, Configuration configuration) {
    for (int i = 0; i < classes.length; i++) {
      ...
      if (configuration.nodeprecated
          && (Util.isDeprecated(classes[i]) || Util.isDeprecated(classes[i].containingPackage()))) {
        continue;
      }

      if (configuration.javafx && classes[i].tags("treatAsPrivate").length > 0) {
        continue;
      }
  ...
  private void processType(
      ...
      List<ClassDoc> bases,
      Map<ClassDoc, List<ClassDoc>> subs) {
  ...
  public List<ClassDoc> implementingclasses(ClassDoc cd) {
    ...
    while (subInterfacesIter.hasNext()) {
      implementingClassesIter = implementingclasses(subInterfacesIter.next()).listIterator();
