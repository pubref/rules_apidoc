...
public class VisibleMemberMap {
   ...
   */
  private final Map<Object, Map<ProgramElementDoc, String>> memberNameMap =
      new HashMap<Object, Map<ProgramElementDoc, String>>();
  ...
  private void purgeMemberLevelMap(List<ProgramElementDoc> list, String level) {
    for (int i = 0; i < list.size(); i++) {
      Object key = getMemberKey(list.get(i));
      Map<ProgramElementDoc, String> memberLevelMap = memberNameMap.get(key);
      if (level.equals(memberLevelMap.get(list.get(i)))) memberLevelMap.remove(list.get(i));
    }
  }
  ...
  private class ClassMembers {
    ...
    private ClassMembers(ClassDoc mappingClass, String level) {
      ...
      if (classMap.containsKey(mappingClass)
        ...
        purgeMemberLevelMap(getClassMembers(mappingClass, false), classMap.get(mappingClass).level);
    ...
    private boolean memberIsVisible(ProgramElementDoc pgmdoc) {
      if (pgmdoc.containingClass().equals(classdoc)) {
        ...
        //same package.
        return pgmdoc.containingClass().containingPackage().equals(classdoc.containingPackage());
      } else {
    ...
    private boolean isOverridden(ProgramElementDoc pgmdoc, String level) {
      ...
      if (memberLevelMap == null) return false;
      ...
      while (iterator.hasNext()) {
        ...
        if (mappedlevel.equals(STARTLEVEL)
            || (level.startsWith(mappedlevel) && !level.equals(mappedlevel))) {
          return true;
        }
      }
      return false;
    ...
    private ProgramElementDoc[] properties(final ClassDoc cd, final boolean filter) {
      ...
      final ProgramElementDoc[] resultAray = result.toArray(new ProgramElementDoc[result.size()]);
      propertiesCache.put(cd, resultAray);
      return resultAray;
    }

    private void addToPropertiesMap(
        MethodDoc setter, MethodDoc getter, MethodDoc propertyMethod, FieldDoc field) {
      if ((field == null)
    ...
    private void addToPropertiesMap(
        ProgramElementDoc propertyMethod, ProgramElementDoc commentSource) {
    ...
    private MethodDoc getterForField(MethodDoc[] methods, MethodDoc propertyMethod) {
      ...
      final String fieldName =
          propertyMethodName.substring(0, propertyMethodName.lastIndexOf("Property"));
      ...
      if ("boolean".equals(fieldTypeName) || fieldTypeName.endsWith("BooleanProperty")) {
          ...
          for (Tag tag : methodDoc.tags()) {
            ...
            if (tagName.equals("@propertySetter")
              ...
              if (!isPropertyGetterOrSetter(members, methodDoc)) {
                configuration.message.warning(tag.position(), "doclet.javafx_tag_misuse");
              }
              break;
            }
          }
  ...
  private Object getMemberKey(ProgramElementDoc doc) {
    if (doc.isConstructor()) {
      ...
      classOrIntName =
          classOrIntName.indexOf('.') != 0
              ? classOrIntName.substring(classOrIntName.lastIndexOf('.'), classOrIntName.length())
              : classOrIntName;
